...
# slider ecomomia despesa administrativa
  ...
  noUiSliderInput(
    inputId = "demais_br",
    label = "% de economia com a redução das demias despesas:",
    min = 0, max = 100,
    value = 15,
    step = 5,
    format = wNumbFormat(thousand = "."))   
...
value <- camaras %>%
        group_by(uf)%>%
        filter (populacao <=  input$corte_uf) %>% 
        summarise(gasto = sum((adm*input$adm_uf/100)+(legis*input$legis_uf/100)+(demais*input$demais_uf/100), na.rm = TRUE))
