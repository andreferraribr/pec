...
# gauge_br 
  gauge_br<- reactive({as.data.frame(map_br()%>% filter (populacao <=  input$corte_br)%>% summarise(pop = sum(populacao), quantidade = n(), gasto = sum(adm+legis, na.rm = TRUE)))})
...
selectInput("uf", label = h3("escolha o Estado:"), 
  ...
  map_uf <- reactive({ map_cidades%>% filter (uf == input$uf) %>% mutate (corte = if_else(populacao >  input$corte_uf , "ok", "mescla"))})
   ...
   gauge_uf<- reactive( {as.data.frame(map_uf()%>% filter (populacao <=  input$corte_uf)%>% group_by (uf) %>% summarise(pop = sum(populacao), quantidade = n(), gasto = sum(adm+legis, na.rm = TRUE)))})
...
"Não serão afetados o Acre, Amapá, Distrito Federal, Rio de Janeiro e Roraima."
})
```
...
value <- camaras %>%
        group_by(uf)%>%
        filter (populacao <=  input$corte_uf) %>% 
        summarise(gasto = sum (adm+legis, na.rm = TRUE))
